...
class PDF extends FPDF
{
    ...
    function LoadData($content)
    {
        ...
        if($content == 0){
            $row = $database->get_results( "SELECT * FROM tb_distribusi_brg AS d JOIN tb_cabang AS c ON c.kode_cabang=d.kode_cabang WHERE d.status=2 LIMIT 0,15" );
        }
    ...
    function Header()
    {
        ...
        $this->Cell(0,30,'Daftar Distribusi Barang',0,0,'C');
        $this->Ln(25);
    }
    // Colored table
    function FancyTable($header, $data)
    {
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell($w[1],6,$row->id_distribusi,'LR',0,'L',$fill);
            $this->Cell($w[2],6,$row->nama_cabang,'LR',0,'L',$fill);
            $this->Cell($w[3],6,$row->tgl_distribusi,'LR',0,'C',$fill);
            $this->Ln();
            $fill = !$fill;
            $i++;
            $y += 10;
    ...
    function Signature($y)
    {
        ...
        $this->Ln(20);
        $this->Cell(0,10,'Bagian Distribusi',0,0,'R');
    }
...
$pdf = new PDF();
// Column headings
$header = array('No', 'Kode Distribusi', 'Nama Cabang', 'Tanggal');
// Data loading
$data = $pdf->LoadData(0);
